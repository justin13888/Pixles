...
import { Trash2, FileVideo, Mail, FileText, ChevronDown, ChevronUp, AppWindow, HelpCircle, Image, RefreshCw } from 'lucide-react';
...
import type { AssetListSearchQueryQuery } from '@/gql/graphql';
...
function Storage() {
  ...
  const [previewAssetId, setPreviewAssetId] = useState<string | undefined>(undefined);
  ...
  return (
    <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="mb-10">
          <h1 className="text-2xl font-medium text-gray-800 mb-4">Manage Storage</h1>
          {
                ...
                : !stats ? <div>No data</div>
                  ...
                  (() => {
                    const storageData = {
                      ...
                      categories: [
                        { name: 'Photos', size: stats.usedStoragePhotos, color: 'bg-red-500' },
                        { name: 'Videos', size: stats.usedStorageVideos, color: 'bg-blue-500' },
                        { name: 'Sidecar', size: stats.usedStorageSidecar, color: 'bg-purple-600' },
                      ],
                      cleanupSuggestions: [
                        {
                          id: 'trash-files',
                    ...
                    const totalPercentageUsed = (storageData.usedStorage / storageData.totalStorage) * 100;

                    return (
                      <>
                        <h2 className="text-xl font-medium text-gray-800 mb-4">
                          {totalPercentageUsed.toFixed(0)}% of storage used ({formatFileSize(storageData.usedStorage)} of {formatFileSize(storageData.totalStorage)})
                        </h2>
                        <p className="text-gray-600 mb-6">
                          Make room for your photos, files, and more by cleaning up space
                        </p>
                        ...
                        <div className="h-2 w-full rounded-full overflow-hidden flex">
                          {storageData.categories.map((category) => (
                            <div
                              ...
                              style={{ width: `${category.size / storageData.totalStorage * 100}%` }}
                            />
                          ))}
                          <div
                            className="h-full bg-gray-200"
                            style={{ width: `${100 - totalPercentageUsed}%` }}
                          />
                        </div>
                        ...
                        <div className="flex flex-wrap mt-4 justify-between">
                          <div className="flex flex-wrap">
                            {storageData.categories.map((category) => (
                              <div key={category.name} className="flex items-center mr-6 mb-2">
                                <div className={cn(
                                ...
                                <span className="text-sm text-gray-700">
                                  {category.name} ({formatFileSize(category.size)})
                                </span>
                              </div>
                            ))}
                          </div>
                          <div className="flex items-center mb-2">
                            ...
                            <span className="text-sm text-gray-700">
                              Free Space ({formatFileSize(storageData.totalStorage - storageData.usedStorage)})
                            </span>
                          </div>
                        </div>
                        ...
                        <div>
                          <h2 className="text-xl font-medium text-gray-800 mb-6">Clean up suggested items</h2>
                          <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as Suggestion)} className="w-full">
                            <TabsList className="mb-4">
                              {
                                storageData.cleanupSuggestions.map((suggestion) => (
                                  <TabsTrigger key={suggestion.id} value={suggestion.id}>
                                    <div className="flex items-center gap-2">
                                      {getIconForSuggestion(suggestion.id)}
                                      {suggestion.title}
                                    </div>
                                  </TabsTrigger>
                                ))
                            ...
                            {
                              storageData.cleanupSuggestions.map((suggestion) => {
                                const [{ data: assetData, fetching: assetFetching, error: assetError }, reexecuteAssetQuery] = useQuery({
                                  query: AssetListSearchQuery,
                                  pause: activeTab !== suggestion.id,
                                });
                                return (
                                  <TabsContent key={suggestion.id} value={suggestion.id}>
                                    <div className="flex flex-col">
                                      {
                                        assetFetching ? <div>Loading...</div>
                                          : assetError ? <div>Error: {assetError.message}</div>
                                            : assetData ? <AssetList assets={assetData.asset.search} />
                                              : <div>No data</div>
                                      }
                                      <div className="flex justify-end mb-4">
                                        <button
                                          type="button"
                                          onClick={() => reexecuteAssetQuery({ requestPolicy: 'network-only' })}
...
// AssetList component
function AssetList({ assets }: { assets: AssetListSearchQueryQuery['asset']['search'] }) {
  if (assets.length === 0) {
  ...
  const columns = [
    ...
    columnHelper.accessor('size', {
      ...
      cell: (info) => <div className="text-right">{formatFileSize(info.getValue())}</div>,
      sortingFn: 'basic',
    }),
    columnHelper.accessor('date', {
      header: () => <div className="text-right">Date</div>,
      cell: (info) => <div className="text-right">{info.getValue().toLocaleDateString()}</div>,
      sortingFn: 'datetime',
    }),
  ];
