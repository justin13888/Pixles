...
pub async fn logout(
    ...
    if let Err(e) = Claims::decode(token_string.expose_secret(), &config.jwt_eddsa_decoding_key) {
        return LogoutResponses::Unauthorized(ClaimValidationError::from(e).into());
    }

    // In a Stateless JWT implementation without a blacklist, there is no server-side action for logout.
    // The client should discard the token.
    // Ideally we would add the token to a blacklist cache (Redis) here.
